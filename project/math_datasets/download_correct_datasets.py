...
def download_dataset(name, repo, description, already_downloaded=False):
    """下载单个数据集"""
    print(f"\n{'='*60}")
    print(f"处理数据集: {name}")
...
def main():
    ...
    for name, config in datasets.items():
        success = download_dataset(
            ...
            config["repo"], 
            config["description"],
            config.get("already_downloaded", False)
        )
